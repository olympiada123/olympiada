...
def olympiads(request):
    ...
    if show_past:
        olympiads_list = (
            Olympiad.objects.filter(is_active=False)
            .prefetch_related("subjects__subject")
            .order_by("-end_date")
        )
    else:
        olympiads_list = Olympiad.objects.filter(is_active=True).prefetch_related(
            "subjects__subject"
        )
    ...
    for olympiad in olympiads_list:
        ...
        subjects_list = [
            ...
            for olympiad_subject in olympiad.subjects.filter(is_active=True)
        ]
...
def olympiad_detail(request, olympiad_id):
    ...
    olympiad_subjects = olympiad.subjects.filter(is_active=True).select_related(
        "subject"
    )
    ...
    if request.user.is_authenticated:
        registration = (
            StudentRegistration.objects.filter(student=request.user, olympiad=olympiad)
    ...
    if (
        ...
        can_register = (
            olympiad.registration_start <= now <= olympiad.registration_end
...
def register_for_olympiad(request, olympiad_id):
    ...
    if user.is_superuser or user.is_staff:
        messages.error(
            request, "Администраторы и кураторы не могут регистрироваться на олимпиады."
        )
    ...
    if not user.student_id:
        messages.error(
            request,
            "Для регистрации на олимпиаду необходимо указать номер студенческого билета в настройках профиля.",
        )
        return redirect("olympiad_detail", olympiad_id=olympiad_id)
    ...
    existing_registration = StudentRegistration.objects.filter(
        student=user, olympiad=olympiad
    ).first()
    ...
    try:
        ...
        if subjects_list:
            registration.subjects.set(
                subjects_list[: olympiad.max_subjects_per_student]
            )

        messages.success(
            request, f'Вы успешно зарегистрированы на олимпиаду "{olympiad.name}".'
        )
...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        if not agree_terms:
            messages.error(
                request, "Необходимо согласиться с правилами и условиями использования."
            )
        ...
        try:
            ...
            messages.success(
                request,
                f"Регистрация успешна! Добро пожаловать, {user.get_full_name() or user.username}!",
            )
            return redirect("index")
        except Exception as e:
...
def profile_view(request):
    ...
    if request.method == "POST" and user.is_superuser:
        if request.POST.get("action") == "update_contact_form_status":
            ...
            if not form_id:
                ...
                if status_filter:
                    ...
                    return redirect(
                        f"{reverse('profile')}?{urlencode({'status_filter': status_filter})}"
                    )
            ...
            try:
                ...
                contact_form.save()
                messages.success(
                    request, "Статус формы обратной связи успешно обновлен."
                )
        ...
        if target_user_id and new_role:
            try:
                ...
                if target_user.id == user.id:
                    messages.error(
                        request, "Вы не можете изменить свою собственную роль."
                    )
                else:
                    if new_role == "admin":
                    ...
                    messages.success(
                        request,
                        f"Роль пользователя {target_user.username} успешно изменена.",
                    )
        ...
        if search_query:
            ...
            return redirect(
                f"{reverse('profile')}?{urlencode({'user_search': search_query})}"
            )
        return redirect("profile")
    ...
    if not user.is_superuser and not user.is_staff:
        registrations = (
            StudentRegistration.objects.filter(student=user)
    ...
    if user.is_staff and not user.is_superuser:
        ...
        curator_olympiads = curator_olympiads_query.prefetch_related(
            "subjects__subject"
...
def settings_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "update_profile":
            ...
            if student_id:
                if (
                    CustomUser.objects.filter(student_id=student_id)
                    .exclude(id=user.id)
                    .exists()
                ):
                    messages.error(
                        request, "Студенческий билет с таким номером уже используется."
                    )
                else:
...
def create_olympiad(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            registration_start_obj = datetime.strptime(
            ...
            registration_start_obj = timezone.make_aware(registration_start_obj)
            registration_end_obj = timezone.make_aware(registration_end_obj)
            ...
            if registration_start_obj >= registration_end_obj:
                ...
                all_subjects = Subject.objects.filter(is_active=True).order_by("name")
                return render(
                    request, "create_olympiad.html", {"subjects": all_subjects}
                )

            if start_date_obj >= end_date_obj:
                messages.error(
                    request, "Дата начала олимпиады должна быть раньше даты окончания."
                )
                all_subjects = Subject.objects.filter(is_active=True).order_by("name")
                return render(
                    request, "create_olympiad.html", {"subjects": all_subjects}
            ...
            if registration_end_obj > start_date_obj:
                messages.error(
                    request, "Регистрация должна завершиться до начала олимпиады."
                )
                all_subjects = Subject.objects.filter(is_active=True).order_by("name")
                return render(
                    request, "create_olympiad.html", {"subjects": all_subjects}
            ...
            olympiad = Olympiad.objects.create(
                ...
                max_subjects_per_student=(
                    int(max_subjects_per_student) if max_subjects_per_student else 1
                ),
            )
                ...
                for subject_id in subject_ids:
                    try:
                        ...
                        OlympiadSubject.objects.create(
                            olympiad=olympiad, subject=subject
                        )
            ...
            messages.error(
                request, f"Произошла ошибка при создании олимпиады: {str(e)}"
            )
...
def edit_olympiad(request, olympiad_id):
    ...
    if request.method == "POST":
        ...
        if not all([name, start_date, end_date, registration_start, registration_end]):
            ...
            all_subjects = Subject.objects.filter(is_active=True).order_by("name")
            current_subjects = [
                os.subject.id for os in olympiad.subjects.filter(is_active=True)
            ]
            return render(
                request,
                "edit_olympiad.html",
        ...
        try:
            ...
            if registration_start_obj >= registration_end_obj:
                ...
                olympiad_subjects = olympiad.subjects.filter(
                    is_active=True
            ...
            olympiad.max_subjects_per_student = (
                int(max_subjects_per_student) if max_subjects_per_student else 1
            )
            ...
            messages.error(
                request, f"Произошла ошибка при обновлении олимпиады: {str(e)}"
            )
...
def manage_questions(request, olympiad_id, subject_id):
    ...
    olympiad_subject = get_object_or_404(
        OlympiadSubject, olympiad=olympiad, subject=subject
    )

    questions = Question.objects.filter(
        olympiad_subject=olympiad_subject, is_active=True
    ).order_by("order")
...
def create_question(request, olympiad_id, subject_id):
    ...
    if request.method == "POST":
        ...
        if not text:
            ...
            return redirect(
                "create_question", olympiad_id=olympiad_id, subject_id=subject_id
            )
        ...
        for i, answer_text in enumerate(answer_texts):
            if answer_text.strip():
                weight = (
                    float(answer_weights[i])
                    if i < len(answer_weights) and answer_weights[i]
                    else 0.0
                )
        ...
        return redirect(
            "manage_questions", olympiad_id=olympiad_id, subject_id=subject_id
        )
...
def start_exam(request, olympiad_id, subject_id):
    ...
    registration = StudentRegistration.objects.filter(
        student=request.user, olympiad=olympiad, subjects=subject
    ).first()
...
def take_exam(request, session_id):
    ...
    if not exam_session.question_order:
        ...
        return redirect(
            "olympiad_detail", olympiad_id=exam_session.olympiad_subject.olympiad.id
        )
    ...
    existing_answer = StudentAnswer.objects.filter(
        exam_session=exam_session, question=question
    ).first()
...
def save_answer(request, session_id):
    ...
    if request.method == "POST":
        ...
        if action == "next":
            ...
            exam_session.questions_answered_count = StudentAnswer.objects.filter(
                exam_session=exam_session
            ).count()
            exam_session.save()
...
def submit_exam(request, session_id):
    ...
    result, created = Result.objects.get_or_create(
        student=request.user, olympiad=olympiad
    )

    all_sessions = ExamSession.objects.filter(
        student=request.user, olympiad_subject__olympiad=olympiad, status="completed"
    )
...
def exam_results(request, session_id):
    ...
    student_answers = (
        StudentAnswer.objects.filter(exam_session=exam_session)
